// 1. Definiere das TypeScript-Interface, um die Struktur zu garantieren
interface LiedData {
  Opus: string;
  Titel_deutsch: string;
  Original_Titel: string;
  Autor: string;
  Komponist: string;
  Interpret: string;
  Genre: string;
  Beziehung_zum_Original: string;
  Entstehung: string;
  YouTube: string;
  TextuAkkorde_pdf: string;
  Aufnahme_mp3: string;
  Soundcloud_mp3: string;
  Noten_pdf: string;
}
 ...
 */
function createLinkCell(url: string, linkText: string): string {
  if (url && url !== "---" && url.trim() !== "") {
 ...
 */
function createDataCell(value: string): string {
...
async function loadAndRenderData() {
  ...
  try {
    ...
    const data: LiedData[] = await response.json();
      ...
      tableHTML += `
                <tr>
                    ...
                    <td>
                        ...
                        <small>Komponist: ${createDataCell(
                          lied.Komponist
                        )}</small>
                    </td>
    ...
    container.innerHTML = `<p style="color: red; text-align: center; padding: 20px;">
            Fehler beim Laden der Daten. Details: ${
